  ...
  <div style="box-sizing: border-box">
    ...
    <div style="padding-bottom: 10px; box-sizing: border-box">
      <el-radio-group v-model="materialType">
        ...
        <el-radio-button :label="2">出库查看单</el-radio-button>
      </el-radio-group>
    </div>
    ...
    <div :class="ns.b('titleBtn')">
      ...
      <div :class="ns.b('btn')">
        <el-button type="danger" plain :disabled="!multipleSelection.length" @click="handleDelete"
          >删除</el-button
        >
        <el-button type="primary" :disabled="!form.value.projectCode" @click="onOutOrderPage"
          >获取</el-button
        >
      </div>
    </div>
    ...
    >
      ...
      <el-table-column prop="productCode" label="样件编码" show-overflow-tooltip />
      <el-table-column prop="productName" label="样件名称" show-overflow-tooltip />
      <el-table-column prop="productBatch" label="样件批次" show-overflow-tooltip />
      <el-table-column prop="totalQty" label="库存数量" show-overflow-tooltip />
      <el-table-column prop="requestedAmount" label="需求数量">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.requestedAmount" :min="0" label="需求数量" />
        </template>
      </el-table-column>
      <el-table-column prop="usableQty" label="可用库存" show-overflow-tooltip />
      <el-table-column prop="torqueRequirement" label="扭矩/其他说明">
        <template slot-scope="scope">
          <el-input v-model="scope.row.torqueRequirement" placeholder="请输入扭矩/其他说明" />
        </template>
      </el-table-column>
      <!-- <el-table-column prop="TSNUmber" label="TS号" show-overflow-tooltip /> -->
      <el-table-column prop="projectCode" label="项目号" show-overflow-tooltip />
      <!-- <el-table-column prop="PE" label="PE" show-overflow-tooltip /> -->
      <el-table-column prop="cargoLocationCode" label="库位号" show-overflow-tooltip />
      ...
      <el-table-column prop="employeeName" label="PE名称" />
    </el-table>
    ...
    <!-- 选择库存弹框 -->
    <el-dialog title="选择库存" :visible.sync="dialogVisible" :append-to-body="true" width="70%">
      <StockTable
        v-if="dialogVisible"
...
import { StockOut as swaggerStockOut } from "@/api/swagger";
...
export default {
      ...
      form: {
        model: [
          ...
          },
          {
            ...
            listeners: {
              change: (item, val) => {
                this.form.value.planDate1 = `${val} 00:00`;
          ...
          },
          {
            type: "select",
            label: "出库查看工作台",
            prop: "outWarehouseStagingId",
            child: [],
            hidden: () => this.materialType !== 2
          }
        ],
        rules: {
          loadingStandard: [
  ...
  watch: {
    ...
    materialType: {
      handler: function (n) {
        // 编辑页面第一次触发监听时候不执行onSerialNo，后面再触发才执行
        if (this.isEditFirst && this.isEdit) {
          this.$nextTick(() => {
            this.isEditFirst = false;
          });
          return;
        }
        if (n !== 1) this.onSerialNo();
        const propsList = ["soNumber", "experimentToNumber", "experimentTtpNumber"];
  ...
  created() {
    if (!Object.keys(this.detailData).length) {
      ...
      this.form.value.planDate1 = moment().format("YYYY-MM-DD HH:mm");
    } else {
      // 编辑
      Object.keys(this.detailData).forEach(key => {
        this.form.value[key] = this.detailData[key];
      });
      // 2025/9/27 上午12:00:00
      this.form.value.planDate1 = "2021-09-27 12:00";
      const enumData = {
        2: 0,
        1: 1,
        4: 2
      };
      this.materialType = enumData[this.detailData.stockOutType];
      this.tableData = this.detailData.stockOutDetailCreateDtos;
    }
  },
  methods: {
    ...
    getSelect() {
      ...
      // 获取查看工作台ID下拉框数据
      this.get({
        url: swaggerStockOut.getStockoutStagingselect
      }).then(res => {
        const tempList = res.result.map(item => {
          return {
            key: item.key,
    ...
    clearHandler() {
      ...
      // this.tableData = [];
      if (this.materialType !== 1) this.onSerialNo();
    ...
    onSubmit: throttle(function () {
      this.$refs.baseFormRef.validate().then(res => {
        ...
        const hasRequestedAmount = this.tableData.some(item => !item.requestedAmount);
        if (hasRequestedAmount) return this.$message.error("请输入需求数量");

        //先写的页面，后对的接口，这里是映射成后端需要的
        const enumData = {
          0: 2,
          1: 1,
          2: 4
        ...
        this.post({
          ...
          data: {
            ...
            stockOutType: enumData[this.materialType]
          }
        }).then(res => {
          if (res.code !== 200) return;
          this.$message({
