...
<template>
  <div>
    ...
    >
      <AddDetail
        v-if="addDetailDialogVisible"
        :type="type"
        :detailData="detailData"
        @submit="onCarBodySubmit"
      />
    ...
    >
      <!-- fullscreen -->
      <AddSampleDetail v-if="addSampleDetailDialogVisible" @submit="onSuccessSampleSubmit" />
    </el-dialog>
...
<script>
import { StockOut, ILT } from "@/api/request/swagger";
...
export default {
  ...
  methods: {
    ...
    editBtnHandler() {
      ...
      if (data.length !== 1) return this.$message.info("请勾选一条数据");
      const row = data[0];
      if (row.orderState !== 1) return this.$message.error("当前单据不能编辑!");
      this.type = 2;
      this.addDetailDialogVisible = true;
      this.detailData = row;
    }
