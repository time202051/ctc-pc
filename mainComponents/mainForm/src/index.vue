  ...
  >
    <template v-for="(item, index) in fieldList">
      <el-form-item
        ...
        :required="requiredHandler(item, item && item.required)"
        :style="getChildWidth(item)"
      >
        <template #label v-if="item.labelRender">
          <RenderComp :createElementFunc="item.labelRender" :item="item"></RenderComp>
        </template>
        ...
        <component
          ...
          :style="{ width: (item && item.attrs && item.attrs.width) || '100%' }"
          :disabled="disabledHandler(item, item && item.attrs && item.attrs.disabled)"
          v-bind="item.attrs || {}"
        ...
        >
          ...
          <template v-else-if="hasChildComponent(item)">
            <component
...
//   options: any[] | string; //二级组件，1.为数字直接使用  2.为string则取receiverMap中找 3.如果没有则 在receiverMap中取prop对应的值
//   value: string;
//   label: string;
...
export default {
  ...
  methods: {
    // 判断是否有二级组件
    hasChildComponent(item) {
      const hasChildComponents = [
        Type.SELECT, // 下拉框
        Type.RADIOGROUP, // 单选组
        Type.CHECKBOXGROUP // 多选组
        // 可以继续扩展其他有二级组件的类型
      ];
      const temp = hasChildComponents.includes(item.type);
      if (temp && !item.childAttrs) item.childAttrs = {};
      return temp || (item.childAttrs && item.childAttrs.options);
    },
    ...
    getOptionsList(item) {
      ...
      if (Array.isArray(options)) {
        ...
        // 没有的时候默认prop
        return this.receiverMap?.[item.prop] || [];
      }
    },

    getPlaceholder(item) {
      // 返回elementui组件的placeholder
      if (Object.keys(item?.attrs).includes("placeholder")) return item.attrs?.placeholder;
    ...
    hideHandler(item) {
      ...
      if (typeof item.hide === "function") return !!item.hide({ item, formData: this.formData });
      return !!item.hide;
    },
    // disabled处理(boolean | function)
    disabledHandler(item, disabled) {
      if (typeof disabled === "function") return !!disabled({ item, formData: this.formData });
      return !!disabled;
    },
    ...
    requiredHandler(item, required) {
      ...
      tempRequired =
        typeof required === "function" ? !!required({ item, formData: this.formData }) : !!required;
